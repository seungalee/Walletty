...
public class AccountAnalyzeService {
    ...
    public String saveAmount(String memberId, String date) { //entity객체는 service에서만
        ...
        for (PaymentEntity pay : paymentAll) {
            ...
            if(date.equals("1") && paydate == 1107 || date.equals("2") && paydate == 1114){
                ...
                if (analyzeEntry.isPresent()){  // 분석 테이블의 해당 주차에 이미 있는 항목이면 해당 항목에 값을 더해서 업데이트 하기
                    ...
                    System.out.println(AccountAnalyzeDTO.toAccountAnalyzeDTO(originEntity));
